...
def recognize_str(most_common, coord_imgs, imgs_with_staff, imgs_spacing, imgs_rows):
    black_names = ['4', '8', '8_b_n', '8_b_r', '16', '16_b_n', '16_b_r',
                   '32', '32_b_n', '32_b_r', 'a_4', 'a_8', 'a_16', 'a_32', 'chord']
    ring_names = ['2', 'a_2']
    whole_names = ['1', 'a_1']
    ...
    for i, img in enumerate(coord_imgs):
        ...
        if len(time_name) == 2:
            ...
            str_result += "[ " + "\\" + "meter<\"" + str(time_name[0]) + "/" + str(time_name[1])+"\">" + ' '.join(
                [str(elem) if type(elem) != list else get_chord_notation(elem) for elem in res]) + "]\n"

        elif len(time_name) == 1:

            str_result += "[ " + "\\" + "meter<\"" + '4' + "/" + '2' + "\">" + ' '.join(
                [str(elem) if type(elem) != list else get_chord_notation(elem) for elem in res]) + "]\n"
        else:
            str_result += "[ " + ' '.join(
                [str(elem) if type(elem) != list else get_chord_notation(elem) for elem in res]) + "]\n"
...
def omr_folder(input_path, output_path):
...
def omr_image(img_path):
        ...
        print("Recognize...")
        result = recognize_str(most_common, coord_imgs,
                  imgs_with_staff, imgs_spacing, imgs_rows)
        print("Done...")
        return result
